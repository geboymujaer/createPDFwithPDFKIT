...
module.exports = function(params)
{
    ...
    async.waterfall([
        ...
        {
            var default_query = "select id, type, billing_id, cust_name, alamat, FORMAT(amount,0,\"de_DE\") as amount, FORMAT(admin,0,\"de_DE\") as admin, FORMAT((amount+admin),0,\"de_DE\") as total_tagihan from data where type = 'PDAM';";
        ...
        function createPDF(data, callback)
        {
            ...
            async.forEachOf(data, function (value, i, callback){ 
                ...
                var filename = params.args.config.path+'PDAM '+value.cust_name+'.pdf';
                ...
                doc.text('ALAMAT', { continued: true });doc.x += 57;
                doc.text(':', { continued: true });doc.x += 5;
                doc.text(value.alamat)
 
                doc.text('BL/TH', { continued: true });doc.x += 69;
                doc.text(':', { continued: true });doc.x += 5;
                doc.text(moment().subtract(1, 'months').format("MMMYY").toUpperCase())

                doc.text('RP TAG PDAM', { continued: true });doc.x += 34;
                doc.text(':', { continued: true });doc.x += 5;
                ...
                doc.moveDown();
                doc.text('PDAM menyatakan struk ini sebagai bukti pembayaran yang sah.', {align : 'center', underline : true})
                doc.moveDown();
