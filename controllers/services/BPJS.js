...
module.exports = function(params)
{
    ...
    async.waterfall([
        ...
        {
            var default_query = "select id, type, billing_id, cust_name, peserta, FORMAT(amount,0,\"de_DE\") as amount, FORMAT(admin,0,\"de_DE\") as admin, FORMAT((amount+admin),0,\"de_DE\") as total_tagihan from data where type = 'BPJS';";
        ...
        function createPDF(data, callback)
        {
            ...
            async.forEachOf(data, function (value, i, callback){ 
                ...
                var filename = params.args.config.path+'BPJS '+value.cust_name+'.pdf';
                ...
                doc.text('JUMLAH PESERTA', { continued: true });doc.x += 15;
                doc.text(':', { continued: true });doc.x += 5;
                doc.text(value.peserta + ' Orang')
 
                doc.text('BL/TH', { continued: true });doc.x += 69;
                doc.text(':', { continued: true });doc.x += 5;
                doc.text(moment().format("MMMYY").toUpperCase())

                doc.text('RP TAG BPJS', { continued: true });doc.x += 40;
                doc.text(':', { continued: true });doc.x += 5;
                ...
                doc.moveDown();
                doc.text('BPJS Kesehatan menyatakan struk ini sebagai bukti pembayaran yang sah.', {align : 'center', underline : true})
                doc.moveDown();
